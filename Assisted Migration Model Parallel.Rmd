...
#RescueMode - source population selection, options below:
...
#Note that, when I move towards the paired tests, we will actually run each population, then split it and have all rescue scenarios run.
...
Paramdf <- expand.grid(RunID = "2024_01_24_Test", Si = 50, K = 500, L = 20, phenvar = 0.3, S0 = 0.75, MuOff = 3, Sel = (1/(1:10)), Mig = c(0, 10^(-5:-1)), EnvType = c("Fixed", "Variable"), timeperiod = 600, deltaGradStart = 500, deltaGradRate = 0.05, deltaGradLength = 100, RescueStart = 550, RescueSites = 0.1, RescueSize = 50, RescueLength = 1, RescueMode = c("None", "Demographic", "Grad", "GradEnv"))
...
donorrecipientsbase <- function(q2, strategy, NumRescueSites, LANDfuture, LANDstarting){
  ...
  drmat[,2] <- recipients
    if(strategy == "None") {
      #this will return just the identities of hypothetical recipients, which can be useful for comparisons later.
    } else if (strategy == "Demographic") {
...
rescuebase <- function(p, donorcplist, K, strategy, RescueSize, NumRescueSites, L, funct2){
  ...
  #If there are no donors, or there are no spots to fill, we just skip to the end and return p
  if (strategy == "None") {
...
#then there will be 40 columns per individual representing 2 alleles each for each of 20 locii - 10 related to Env, 10 related to Grad, the first 10 are the Grad from mom, next 10 are the Grad from dad, next 10 are the Env from mom, next 10 are the Env from dad
  ...
  #landgradovertime <- matrix(NA, nrow = PSL$timeperiod, ncol = PSL$Si)  
  #landenvovertime <- matrix(NA, nrow = PSL$timeperiod, ncol = PSL$Si)
  popovertime <- matrix(NA, nrow = PSL$timeperiod, ncol = PSL$Si)
  Gradovertime <- matrix(NA, nrow = PSL$timeperiod, ncol = PSL$Si)
  Envovertime <- matrix(NA, nrow = PSL$timeperiod, ncol = PSL$Si)
  Gradvarovertime <- matrix(NA, nrow = PSL$timeperiod, ncol = PSL$Si)
  Envvarovertime <- matrix(NA, nrow = PSL$timeperiod, ncol = PSL$Si)
  
    #Simulate population and evolutionary dynamics with these parameters
  for (t in 1:PSL$timeperiod) {
    ...
    #Check if it is time for rescue & implement
    if (t == PSL$RescueStart) {donorrecipientslist <- donorrecipients(q2 = POPsim, 
      strategy = PSL$RescueMode, NumRescueSites = PSL$NumRescueSites, LANDfuture = LANDsimfuture, LANDstarting = LANDsimstarting)}
    if ((t >= PSL$RescueStart) & (t < PSL$RescueStart + PSL$RescueLength)) {POPsim <- rescue(p = POPsim, donorcplist = donorrecipientslist, K = PSL$K, strategy = PSL$RescueMode, RescueSize = PSL$RescueSize, NumRescueSites = PSL$NumRescueSites, L = PSL$L, funct2 = funct)}
  ...
  popovertimedf <- data.frame(Site = rep(1:PSL$Si, each = PSL$timeperiod), Time = rep(1:PSL$timeperiod, times = PSL$Si), Variable = "Pop.Size", Value = as.vector(popovertime))
  gradovertimedf <- data.frame(Site = rep(1:PSL$Si, each = PSL$timeperiod), Time = rep(1:PSL$timeperiod, times = PSL$Si), Variable = "Grad", Value = as.vector(Gradovertime))
  envovertimedf <- data.frame(Site = rep(1:PSL$Si, each = PSL$timeperiod), Time = rep(1:PSL$timeperiod, times = PSL$Si), Variable = "Env", Value = as.vector(Envovertime))
  gradvarovertimedf <- data.frame(Site = rep(1:PSL$Si, each = PSL$timeperiod), Time = rep(1:PSL$timeperiod, times = PSL$Si), Variable = "Grad.Var", Value = as.vector(Gradvarovertime))
  envvarovertimedf <- data.frame(Site = rep(1:PSL$Si, each = PSL$timeperiod), Time = rep(1:PSL$timeperiod, times = PSL$Si), Variable = "Env.Var", Value = as.vector(Envvarovertime))
  dataovertimedf <- rbind(popovertimedf, gradovertimedf, envovertimedf, gradvarovertimedf, envvarovertimedf)

  #Check is donorrecipientslist exists, if not (for example, because the population went extinct before RescueStart), create a null
  if (exists("donorrecipientslist") == F) {
    donorrecipientslist <- matrix(NA_integer_, ncol = 2, nrow = PSL$NumRescueSites)
    }
...
set.seed(2076)
results_mclapply <- mclapply(Paramlist, simfunc, mc.cores = 15)
