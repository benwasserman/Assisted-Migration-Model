...
donorrecipientsbase <- function(q2, strategies, NumRescueSites, LANDfuture, LANDstarting, NumRescueSources){
  ...
  #A function for matching from multiple sources
   minNum <- function(vec, num = NumRescueSources) {
            nonNA <- vec[which(!is.na(vec))] 
            nonNAindex <- rank(nonNA, ties = "random")
            minindex <- rep(NA_integer_, num)
            minindex[1:min(length(nonNAindex),num)] <- as.numeric(names(nonNAindex))[which(nonNAindex %in% 1:num)]
            return(minindex)
        }
  
  for (qi in 1:7){#length(strategies[[1]])){
    if(strategies[[1]][qi] == "None") {
      ...
      colnames(drmat)[1+qi] <- strategies[[1]][qi]
        ...
        drmat[,(1+qi):(qi+NumRescueSources)] <- matrix(eligibledonors[knnx.index(eligibledonors, drmat[,1], k = NumRescueSources)], nrow = nrow(drmat), ncol = NumRescueSources, byrow = FALSE)
