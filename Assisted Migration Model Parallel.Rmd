...
rescuebase <- function(p, donorcplist, K, strategy, RescueSize, NumRescueSites, L, funct2, NumRescueSources){
  ...
  if (strategy == "Multiple") {
    donorcplist2 <- matrix(NA, nrow = nrow(donorcplist)*(ncol(donorcplist)-1), ncol = 2)
    donorcplist2[,1] <- rep(donorcplist[,1], each = (ncol(donorcplist)-1))
    donorcplist2[,2] <- as.vector(t(donorcplist[,2:ncol(donorcplist)]))
    donorcplist <- donorcplist2
  }
  ...
  recipientpopsizes <- popsizes[donorcplist[,1]]
  recipientopportunities <- pmin(rep(RescueSize,NumRescueSites),(K-popsizes[donorcplist[,1]]))
  donorsizes <- popsizes[donorcplist[,2]]
  #donoropportunities
  ...
  #changing times = floor(recipientopportunities/NumRescueSources) should a) keep the total number of 
  
  if(strategy == "Multiple"){
    offspringmoved <- matrix(NA, nrow = sum(floor(recipientopportunities/NumRescueSources)), ncol = 11+2*L)
    offspringmoved[,1] <- rep(donorcplist[,2], times = floor(recipientopportunities/NumRescueSources))
    offspringmoved[,2] <- rep(donorcplist[,1], times = floor(recipientopportunities/NumRescueSources))
  } else {
    offspringmoved <- matrix(NA, nrow = sum(recipientopportunities, na.rm = TRUE), ncol = 11+2*L)
    offspringmoved[,1] <- rep(donorcplist[,2], times = recipientopportunities)
    offspringmoved[,2] <- rep(donorcplist[,1], times = recipientopportunities)
  }
...
#Now outside of either of the two loops (first sesh and second sesh, I'll pick the rescue)
  ...
  donorrecipientslist <- matrix(NA_integer_, ncol = 1+length(PSL$RescueModes[[1]])+sum(PSL$RescueModes[[1]]=="Multiple")*(PSL$RescueSourceNumber-1), nrow = PSL$NumRescueSites) #this assumes 1 row per recipient, we'll resave the empty matrix if strategy = "multiple"
