...
Paramdfsingle <- expand.grid(RunID = "A_2026_05_06_omega=20", Si = 50, K = 500, L = 20, phenvar = 0.3, S0 = 0.75, MuOff = 3, Sel = (1/c(1:10,20)), Mig = c(0, 10^(-5:-1)), EnvType = c("Fixed", "Variable"), timeperiod = 600, deltaGradStart = 500, deltaGradRate = 0.05, deltaGradLength = 100, RescueModes = list(c("None", "Demographic", "Grad", "GradEnv", "Phenotype", "Multiple", "GradAdj", "GradEnvAdj")), RescueStart = 550, RescueSites = 0.1, RescueSize = 50, RescueLength = 1, RescueSourceNumber = 5)
...
#2026-04-15 Adding two more strategies based on mixing popualtions sensu Prober et al 2015
#GradAdj: rescue based on a mix of 5 populations along the climate gradient
#GradEnvAdj: rescue based on a mix of 5 populations based on the bivariate distance 
...
donorrecipientsbase <- function(q2, strategies, NumRescueSites, LANDfuture, LANDstarting, NumRescueSources){
  ...
  drmat <- matrix(NA_integer_, ncol = 1+length(strategies[[1]])+sum(strategies[[1]]%in%c("Multiple", "GradAdj", "GradEnvAdj"))*(NumRescueSources-1), nrow = length(recipients)) #this assumes 1 row per recipient, we'll resave the empty matrix if strategy = "multiple"
  drmat[,1] <- recipients
  colnames(drmat) <- rep(NA, ncol(drmat))
  colnames(drmat)[1] <- "Recipients"
  
  #A function for matching from multiple sources
   minNum <- function(vec, num) {
            nonNA <- vec[which(!is.na(vec))] 
            nonNAindex <- rank(nonNA, ties = "random")
            minindex <- as.numeric(names(nonNAindex)[which(nonNAindex %in% 1:NumRescueSources)])
            return(minindex)
        }
  ...
  for (qi in 1:length(strategies[[1]])){
    if(strategies[[1]][qi] == "None") {
      ...
      colnames(drmat)[1+qi] <- strategies[[1]][qi]
        ...
        drmat[,(1+qi):(qi+NumRescueSources)] <- matrix(eligibledonors[knnx.index(eligibledonors, drmat[,1], k = min(NumRescueSources, length(eligibledonors), na.rm = TRUE))], nrow = nrow(drmat), ncol = NumRescueSources, byrow = FALSE)
        colnames(drmat)[(1+qi):(qi+NumRescueSources)] <- rep(strategies[[1]][[qi]], NumRescueSources)
    } else if (strategies[[1]][qi] == "GradAdj"){
        
        #Set up a distance matrix from donors to recipient sites
        distlong <- cbind(rep(LANDstarting$Grad[eligibledonors],NumRescueSites), c(rep(((LANDstarting$Grad+LANDfuture$Grad)/2)[drmat[1,1]],length(eligibledonors)), 
  rep(((LANDstarting$Grad+LANDfuture$Grad)/2)[drmat[2,1]],length(eligibledonors)),
  rep(((LANDstarting$Grad+LANDfuture$Grad)/2)[drmat[3,1]],length(eligibledonors)),
  rep(((LANDstarting$Grad+LANDfuture$Grad)/2)[drmat[4,1]],length(eligibledonors)),
  rep(((LANDstarting$Grad+LANDfuture$Grad)/2)[drmat[5,1]],length(eligibledonors))),rep(NA, length(drmat[,1]*length(eligibledonors))))
        distlong[,3] <- abs(distlong[,2]-distlong[,1])
        colnames(distlong) <- c("DonorStart", "Midpoint", "Match")
      
        
        
        distmat <- matrix(data = distlong[,3], nrow = NumRescueSites, ncol = length(eligibledonors),byrow = TRUE)
        rownames(distmat) <- drmat[,1]
        colnames(distmat) <- eligibledonors
        
        #choose the 5 best matches for each recipient 
        
        drmat[,(qi+NumRescueSources):(qi-1+2*(NumRescueSources))] <- t(apply(distmat, MARGIN = 1, FUN = minNum, simplify = TRUE))
        
     
        colnames(drmat)[(qi+NumRescueSources):(qi-1+2*NumRescueSources)] <- rep(strategies[[1]][[qi]], NumRescueSources)
     } else if (strategies[[1]][qi] == "GradEnvAdj"){
      
       #set up a distance matrix that accounts for grad and environment
         
       distlong2 <- cbind(rep(LANDstarting$Grad[eligibledonors],NumRescueSites), rep(LANDstarting$Env[eligibledonors], NumRescueSites),
           c(rep(((LANDstarting$Grad+LANDfuture$Grad)/2)[drmat[1,1]],length(eligibledonors)), 
  rep(((LANDstarting$Grad+LANDfuture$Grad)/2)[drmat[2,1]],length(eligibledonors)),
  rep(((LANDstarting$Grad+LANDfuture$Grad)/2)[drmat[3,1]],length(eligibledonors)),
  rep(((LANDstarting$Grad+LANDfuture$Grad)/2)[drmat[4,1]],length(eligibledonors)),
  rep(((LANDstarting$Grad+LANDfuture$Grad)/2)[drmat[5,1]],length(eligibledonors))),
    c(rep(((LANDstarting$Env+LANDfuture$Env)/2)[drmat[1,1]],length(eligibledonors)),
      rep(((LANDstarting$Env+LANDfuture$Env)/2)[drmat[2,1]],length(eligibledonors)),
  rep(((LANDstarting$Env+LANDfuture$Env)/2)[drmat[3,1]],length(eligibledonors)),
  rep(((LANDstarting$Env+LANDfuture$Env)/2)[drmat[4,1]],length(eligibledonors)),
  rep(((LANDstarting$Env+LANDfuture$Env)/2)[drmat[5,1]],length(eligibledonors))),
  rep(NA, length(drmat[,1]*length(eligibledonors))))
       
      distlong2[,5] <- sqrt((distlong2[,3]-distlong2[,1])^2+(distlong2[,4]-distlong2[,2])^2)
        colnames(distlong2) <- c("DonorStartGrad", "Env", "MidpointGrad", "MidpointEnv", "Match")
        
        
        distmat2<- matrix(data = distlong2[,5], nrow = NumRescueSites, ncol = length(eligibledonors),byrow = TRUE)
        rownames(distmat2) <- drmat[,1]
        colnames(distmat2) <- eligibledonors
  
        #choose the 5 best matches for each recipient       
        
        drmat[,(qi-1+2*NumRescueSources):dim(drmat)[2]] <- t(apply(distmat2, MARGIN = 1, FUN = minNum, simplify = TRUE))
        colnames(drmat)[(qi-1+2*(NumRescueSources)):dim(drmat)[[2]]]<-rep(strategies[[1]][[qi]], NumRescueSources)
    }
  
    ##For strategies GradAdj and GradEnvAdj I need to take the functions I wrote for Grad and Env and rather than just taking the maximum, I need to look at that distance matrix and evenly space 5 populations between the max and the current. Actually what if I use the knnx.index function on the landscape values rather than the index values. That's the nxt thing to try
...
rescuebase <- function(p, donorcplist, K, strategy, RescueSize, NumRescueSites, L, funct2, NumRescueSources){
  #First condense the strategies with multiple column donorcplist into a long format   
  #2026_04_28 what I have to do in this if statement, is change strategy == to strategy %in% c(...)
  #The I have to correct the instructions for donorcplistlong[,1&2] to identify those columns with a name that is equal to the column title
  
  if (strategy %in% c("Multiple", "GradAdj", "GradEnvAdj")) {
  ...
  #Identify and add mutations to the movers
      mutationsmovers <- matrix(data = rbinom(n = 2*L*dim(offspringmoved)[1], size = 1, prob = 0.0001), ncol = 2*L)
...
#Now outside of either of the two loops (first sesh and second sesh, I'll pick the rescue)
  ...
  donorrecipientslist <- matrix(NA_integer_, ncol = 1+length(PSL$RescueModes[[1]])+sum(PSL$RescueModes[[1]]%in%c("Multiple","GradAdj","GradEnvAdj"))*(PSL$RescueSourceNumber-1), nrow = PSL$NumRescueSites) 
  RescueModesSS <- case_when(PSL$RescueModes[[1]] %in% c("None", "Demographic", "Grad", "GradEnv", "Phenotype") ~ 1,
                             PSL$RescueModes[[1]] %in% c("Multiple", "GradAdj", "GradEnvAdj") ~ PSL$NumRescueSites)
  colnames(donorrecipientslist) <- c("Recipients", rep(PSL$RescueModes[[1]],times = RescueModesSS))
}
