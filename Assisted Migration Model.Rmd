...
for(t in 1:timeperiod){
  ...
  Gradvarovertime[t,] <- colSums((POPS[,8,]-matrix(rep(Gradovertime[t,],each = K0),nrow = K))^2,na.rm = TRUE)/colSums(!is.na(POPS[,8,]), na.rm = TRUE)
...
simfunc <- function(Si = 50, K = 500, L = 20, EnvType, phenvar = 0.3, Omega2grad, Omega2env, S0 = 0.75, MuOff = 2, MuMovement, timeperiod = 600, deltaGradStart = 501, deltaGradRate = 0.05, deltaGradLength = 100) {
  ...
  if (EnvType == "Variable") {LAND <- data.frame(Grad = 1:Si, Env = sample(1:Si, replace = TRUE))
  } else if (EnvType == "Fixed") {LAND <- data.frame(Grad = 1:Si, Env = Si/2)}
...
for(t in 1:timeperiod){
  ...
  print(environment())
  ls(environment())
  POPsim <- eval(death(POPsim),environment())
  ...
  enddata <- matrix(NA, nrow = 4, ncol = Si)
  ...
  #Phenotypic variance
  PhenotypicVariance <- colSums(POPsim)
