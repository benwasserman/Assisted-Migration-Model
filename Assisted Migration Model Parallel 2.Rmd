---
title: "Assisted Migration Model Parallel Processing 2nd Ed"
author: "Ben Wasserman"
date: "`r Sys.Date()`"
output: html_document
---

1st edition included the following rescue scenarios: 
...
Test2nd Edition is going to be whether rescuing based on current value of GRAD/ENV rather than ending is important. Like maybe things are tracking well and we're throwing them off.
So run A2025_07_11 uses donors whose starting values are most like the current value of the grad and/or environment rather than most like the ending future value (which is what I've used in all previous simulations) 

Test 3rd Edition (to be run in a different script) - will revert back. That way I can do the comparison
#Setup
...
Paramdfsingle <- expand.grid(RunID = "2_A_2025_07_12", Si = 50, K = 500, L = 20, phenvar = 0.3, S0 = 0.75, MuOff = 3, Sel = (1/(1:10)), Mig = c(0, 10^(-5:-1)), EnvType = c("Fixed", "Variable"), timeperiod = 600, deltaGradStart = 500, deltaGradRate = 0.05, deltaGradLength = 100, RescueModes = list(c("None", "Demographic", "Grad", "GradEnv", "Multiple")), RescueStart = 550, RescueSites = 0.1, RescueSize = 50, RescueLength = 1, RescueSourceNumber = 5)
...
#Now outside of either of the two loops (first sesh and second sesh, I'll pick the rescue)
  donorrecipientslist <- donorrecipients(q2 = POPsim, strategies = PSL$RescueModes, NumRescueSites = PSL$NumRescueSites, LANDfuture = LANDsim, LANDstarting = LANDsimstarting, NumRescueSources = PSL$RescueSourceNumber)
  #2025_07_11 in 2nd Ed: to test whether match should be to future or current, i am replacing in the line above this "LANDfuture = LANDsimfuture" with "LANDfuture = LANDsim" 
  #2025_07_12 in 3rd Ed: to test whether match should be to future or current, I reverted these lines back
...
#set.seed(2076)
results_mclapply <- mclapply(Paramlist, simfunc, mc.preschedule = FALSE, mc.cores = 7)
...
save(Paramdf, results_mclapply, file = paste0("Outputs/Parallel/", Paramdf$RunID[1], "_Outputs_2nd_Ed.Rdata"))
beep(sound = 6)
```
